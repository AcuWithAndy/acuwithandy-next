...
export type Post = { _id: string; title: string; slug: string; excerpt: string; publishedAt: string; author: string; cover?: { url: string; alt: string }; body: Block[] };
...
export const getPosts = cache(async (): Promise<Post[]> => {
  ...
  const query = `*[_type == "post" && !(_id in path("drafts.**")) && defined(slug.current) && defined(publishedAt) && dateTime(publishedAt) <= dateTime(now())] | order(publishedAt desc) {_id,title,"slug":slug.current,excerpt,publishedAt,"author":coalesce(author,"Andy Boehm, LAc."),"cover":cover{alt,"url":asset->url},body}`;
...
export function readTime(body: Block[] = []) {
